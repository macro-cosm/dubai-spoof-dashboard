...
def generate_vix_simulation_data(n_samples=100, seed=42):
    """
    Generate VIX simulation data and save to CSV:
    - Calibration period starts March 14, 2024
    - Average of samples = VIX Close + 3% Gaussian noise (becomes the mean)
    ...
    vix_data = vix_data[vix_data.index >= calibration_start]

    # Split data into calibration and forecasting periods
    calibration_data = vix_data[vix_data.index <= cutoff_date]["Close"]
    forecasting_data = vix_data[vix_data.index > cutoff_date]["Close"]

    print(
        f"Data from calibration start: {len(vix_data)} days (from {calibration_start.strftime('%Y-%m-%d')})"
    )
    print(
    ...
    # Mean = VIX Close + 3% Gaussian noise (this becomes the average for all samples)
    ...
    # Generate daily means: actual VIX + 3% noise
    daily_means = []
    ...
    # Save to CSV
    output_file = "data/spoof_vix_data.csv"
    final_df.to_csv(output_file)

    print(f"\n✅ VIX simulation data saved to {output_file}")
    ...
    if calib_mask.any():
        calib_actual = final_df.loc[calib_mask, "Actual_VIX"]
    ...
    if forecast_mask.any():
        forecast_actual = final_df.loc[forecast_mask, "Actual_VIX"]
...
data, cutoff_date = generate_vix_simulation_data()
...
if __name__ == "__main__":
    try:
        ...
        print("✅ VIX simulation generation completed successfully!")
    except Exception as e:
        print(f"❌ Error generating VIX simulation data: {str(e)}")
