...
def create_order_flow_plots(volume_data, rates_data):
    ...
    fig.update_xaxes(gridcolor="#404040", zerolinecolor="#404040", color="#ffffff", showticklabels=False)
    fig.update_yaxes(gridcolor="#404040", zerolinecolor="#404040", color="#ffffff", showticklabels=False)
    fig.update_xaxes(range=[0, 6], row=1, col=1)
    fig.update_xaxes(range=[0, 15], row=2, col=1)
...
def create_risk_index_plot(actual_data, sim_data, cutoff_date, confidence_level=95):
    ...
    fig.update_layout(
        title={"text": "Total Risk Index", "x": 0.5, "xanchor": "center", "font": {"size": 16, "color": "#ffffff"}},
        xaxis_title="Date",
        yaxis_title="Risk Index",
        hovermode="x unified",
        legend=dict(
            yanchor="top", y=0.99, xanchor="left", x=0.01,
            bgcolor="rgba(45, 45, 45, 0.9)", bordercolor="#ffffff", borderwidth=1,
            font=dict(color="#ffffff", size=10)
        ),
        plot_bgcolor="#1e1e1e",
        paper_bgcolor="#1e1e1e",
...
def create_market_depth_plot(actual_data, sim_data, cutoff_date, confidence_level=95):
    ...
    fig.update_layout(
        title={"text": "Market Depth Index", "x": 0.5, "xanchor": "center", "font": {"size": 16, "color": "#ffffff"}},
        xaxis_title="Date",
        ...
        font=dict(color="#ffffff"),
        xaxis=dict(gridcolor="#404040", zerolinecolor="#404040", color="#ffffff"),
        yaxis=dict(gridcolor="#404040", zerolinecolor="#404040", color="#ffffff", tickformat=".2s"),
        height=300,
        margin=dict(l=50, r=50, t=50, b=50)
    )
...
def main():
    ...
    try:
        ...
        risk_actual_data, risk_sim_data, risk_cutoff_date, risk_calibration_data, risk_forecasting_data = load_risk_simulation_data()
        
        # Load market depth data
        depth_actual_data, depth_sim_data, depth_cutoff_date, depth_calibration_data, depth_forecasting_data = load_depth_simulation_data()
        ...
        **Market Depth Model**
        ...
        with col2:
            ...
            risk_fig = create_risk_index_plot(risk_actual_data, risk_sim_data, risk_cutoff_date, confidence_level)
            st.plotly_chart(risk_fig, use_container_width=True)
            
            # Bottom: Market Depth Index
            st.markdown("### Market Depth Index")
            depth_fig = create_market_depth_plot(depth_actual_data, depth_sim_data, depth_cutoff_date, confidence_level)
